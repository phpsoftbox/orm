...
Колонку, значение которой вычисляет база данных (`DEFAULT`, trigger или
generated column), можно отобразить как read-only:
...
ORM будет гидратировать такое свойство при чтении, но исключит колонку из
`INSERT` и `UPDATE`. Для новой сущности свойство следует инициализировать,
поскольку mapper извлекает состояние всех mapped-свойств.

После записи вычисленное базой значение автоматически не перечитывается.
Если оно нужно в текущем объекте, вызовите:

```php
$entityManager->refresh($entity);
```

`refresh()` сохраняет текущий managed instance, повторно гидратирует строку через
репозиторий и переносит в него mapped columns. Поэтому применяются те же casts,
что и при обычном ORM-чтении (`bool`, date/time и `DatePoint`, enum, JSON, UUID,
nullable-типы и касты custom repository). После обновления ORM пересоздаёт snapshot
и сбрасывает loaded-state relations.

Если generated-колонка нужна только как технический маркер для индекса,
отображать её в entity необязательно. Атрибут `GeneratedValue` предназначен
только для генерации идентификаторов и не описывает generated columns.
