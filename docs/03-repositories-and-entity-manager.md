...
Слушатель `OnBulkRemove` может дописать в этот `UPDATE` свои колонки через `state()->register()`; для поштучного
`remove()` то же самое делает слушатель `OnDelete` — см.
[Behaviors, события и DI](05-behaviors-events-di.md).
