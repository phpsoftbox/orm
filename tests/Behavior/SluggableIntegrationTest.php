...
use PhpSoftBox\Orm\Tests\Behavior\Fixtures\PostWithMappedSlug;
...
final class SluggableIntegrationTest extends TestCase
{
    ...
    /**
     * Проверяет, что #[Sluggable] принимает имена свойств и сохраняет значение в замапленную колонку.
     */
    #[Test]
    public function sluggableResolvesMappedSourceAndTargetColumns(): void
    {
        $pdo = new PDO('sqlite::memory:');
        ...
        $post = new PostWithMappedSlug(id: 3, title: 'Mapped Title', seoSlug: '');

        $em->persist($post);
        $em->flush();

        $row = $conn->fetchOne('SELECT seo_slug FROM posts_mapped WHERE id = 3');
        self::assertNotNull($row);
        self::assertSame('3-mapped-title', $row['seo_slug']);
        self::assertSame('3-mapped-title', $post->seoSlug);
    }
}
