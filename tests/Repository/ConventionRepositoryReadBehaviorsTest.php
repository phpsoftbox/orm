...
final class ConventionRepositoryReadBehaviorsTest extends TestCase
{
    ...
    public function conventionRepositoryUsesReadBehaviors(): void
    {
        ...
        self::assertInstanceOf(SoftDeleteAwareEntityRepositoryInterface::class, $repo);
        self::assertTrue($repo->existsWithDeleted(2));

        $foundWithDeleted = $em->findWithDeleted(SoftDeleteUser::class, 2);
        self::assertInstanceOf(SoftDeleteUser::class, $foundWithDeleted);
        self::assertSame($foundWithDeleted, $em->findWithDeleted(SoftDeleteUser::class, 2));
    }
}
