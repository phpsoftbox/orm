...
use PhpSoftBox\Orm\Tests\Relations\Fixtures\Repository\NonBulkRoleRepository;
...
use PhpSoftBox\Orm\Tests\Relations\Fixtures\Repository\UserWithScopedRolesRepository;
use PhpSoftBox\Orm\Tests\Relations\Fixtures\Role;
use PhpSoftBox\Orm\Tests\Relations\Fixtures\UserWithRoles;
use PhpSoftBox\Orm\Tests\Relations\Fixtures\UserWithScopedRoles;
...
final class BelongsToManyIntegrationTest extends TestCase
{
    ...
    /**
     * Проверяет, что BelongsToMany поддерживает scope на target-таблице и отдельный scope на pivot-таблице.
     */
    #[Test]
    public function eagerLoadsBelongsToManyWithTargetAndPivotScopes(): void
    {
        $conn = $this->createConnection();
        ...
        $em->registerRepository(UserWithScopedRoles::class, new UserWithScopedRolesRepository($conn, $em));
        $em->registerRepository(Role::class, new RoleRepository($conn, $em));

        $repo = $em->repository(UserWithScopedRoles::class);
        self::assertInstanceOf(AbstractEntityRepository::class, $repo);

        /** @var AbstractEntityRepository<UserWithScopedRoles> $repo */
        $users = $repo->with(['roles'])->all();
        $items = $users->all();

        self::assertCount(1, $items);
        self::assertCount(1, $items[0]->roles->all());
        self::assertSame(['admin'], array_map(fn (Role $r) => $r->name, $items[0]->roles->all()));
        self::assertSame([10], array_map(fn (Role $r) => $r->id, $items[0]->roles->all()));
    }

    /**
     * Проверяет fallback: если репозиторий target-сущности не Bulk, ORM использует GenericEntityRepository.
     */
    #[Test]
    public function eagerLoadsBelongsToManyWithNonBulkRepository(): void
    {
        $conn = $this->createConnection();
        ...
        $em->registerRepository(UserWithRoles::class, new UserWithRolesRepository($conn, $em));
        $em->registerRepository(Role::class, new NonBulkRoleRepository());
