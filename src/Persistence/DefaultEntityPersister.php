...
final readonly class DefaultEntityPersister implements EntityPersisterInterface
{
    ...
    public function delete(EntityInterface $entity, array $additionalColumns = []): void
    {
        ...
        if ($meta->softDelete !== null) {
            ...
            // Колонку soft delete задаёт ORM, поэтому она пишется последней.
            $data = $this->filterUpdatableColumns($meta, $pk, $additionalColumns);

            $data[$meta->softDelete->column] = $deletedValue;

            $connection
                ->query()
                ->update($meta->table, $data)
                ->where($pk . ' = :__orm_pk', ['__orm_pk' => $idValue])
                ->execute();
    ...
    /**
     * Оставляет только обновляемые колонки сущности, кроме pk.
     *
     * @param array<string, mixed> $data
     * @return array<string, mixed>
     */
    private function filterUpdatableColumns(ClassMetadata $meta, string $pk, array $data): array
    {
        $filtered = [];
