 ...
 */
abstract class AbstractEntityRepository extends AbstractRepository implements BulkEntityRepositoryInterface, SoftDeleteAwareEntityRepositoryInterface
{
    /**
    ...
    public function existsWithDeleted(int|UuidInterface|array|IdentifierInterface $id): bool
    {
        ...
        $row = $this->query(withDeleted: true)
    ...
    private function findInternal(
        int|UuidInterface|array|IdentifierInterface $id,
        bool $withDeleted,
    ): ?EntityInterface {
        $where = $this->normalizeIdentifier($id);

        $row = $this->query(withDeleted: $withDeleted)
            ->where($where['sql'], $where['params'])
            ->fetchOne();

        $entity = $row === null ? null : $this->hydrate($row);

        if ($entity !== null && $this->em !== null) {
            $entity = $this->em->manageHydratedEntities([$entity])->first();
        }
    ...
    public function all(): EntityCollection
    {
        ...
        if ($entities !== [] && $this->em !== null) {
            $entities = $this->em->manageHydratedEntities($entities)->all();
        }
    ...
    public function findManyByColumn(array $ids, string $column = 'id', bool $withDeleted = false): EntityCollection
    {
        return $this->findManyByLookup(
            LookupSpec::forTable($this->table())->lookupColumn($column)->values($ids),
            withDeleted: $withDeleted,
        );
    }

    /**
     * Батч-загрузка по общей keyed lookup-спецификации.
     *
     * @return EntityCollection<TEntity>
     */
    public function findManyByLookup(LookupSpec $lookup, bool $withDeleted = false): EntityCollection
    {
        ...
        if ($lookup->tableName() !== $this->table()) {
            throw new InvalidArgumentException('Lookup table does not match repository table.');
        }

        $query = $this->query(withDeleted: $withDeleted);
        $this->applyLookupCriteria($query, $lookup->whereCriteria());
        $rows = $query
            ->whereIn($lookup->lookupColumnName(), $lookup->lookupValues())
            ->fetchAll();
     ...
     */
    private function applyLookupCriteria(SelectQueryBuilder|OrmSelectQueryBuilder $query, array $criteria): void
    {
        $index = 0;
     ...
     */
    protected function query(bool $withDeleted = false): SelectQueryBuilder|OrmSelectQueryBuilder
    {
        if ($this->em !== null) {
